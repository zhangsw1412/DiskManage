...
Class Image {
	 ...
	 * @return boolean
	 */
	Static Private function checkCondition ($file = NULL) {//echo 233;var_dump(extension_loaded('GD'));die;
		return is_null($file) 
		?
		extension_loaded('GD') && function_exists('imagecreatetruecolor') && function_exists('imagepng') 
		: 
		extension_loaded('GD') && file_exists($file);
	}
}
?>